...
|------------|   |------------|     |--------------|   |------------|
|addMember() |   |payBill()   |     |logVisitor()  |   |genBills()  |
|manageUnit()|   |vote()      |     |markStaffAtt()|   |recExpense()|
|genReports()|   |sendSOS()   |     |respondToSOS()|   |genReports()|
|createPoll()|   |preApprove()|     |manageGate()  |   |            |
+------------+   +------------+     +--------------+   +------------+
      | 1              | M                 | 1               | 1
      |                |                   |                 |
      | manages        | resides in        | monitors        | generates
      | 0..*           | 1                 | 0..*            | 0..*
+------------+   +------------+     +--------------+   +------------+
|    Unit    |   | Ownership  |     |   SOSAlert   |   |    Bill    |
|------------|   |   Record   |     |--------------|   |------------|
|unitId      |   |------------|     |alertId       |   |billId      |
|status      |   |startDate   |     |status        |   |amount      |
|------------|   |endDate     |     |--------------|   |------------|
|updateUnit()|   |trackHist() |     |triggerAlert()|   |genBill()   |
+------------+   +------------+     +------+-------+   +-----+------+
                                           | 1               | 1
                                           |                 | 0..1 (Comp)
                                    +------+-------+   +-----+------+
